...
import yaml
import gc
from stage3.checkpoint_utils import (
    load_checkpoint_memory_efficient, 
    load_model_only, 
    save_checkpoint_compressed,
    get_checkpoint_info
)
from stage3.model import DIV_fusion_model,Degrad_restore_model
    ...
    for batch_idx, (i_image, v_image,d_image) in enumerate(pbar):
        try:
            if model.task=="dv_i":
                ...
                clean_fusion=clean_fusion.to(device_3)
                i_image=i_image.to(device_3)
                v_image=v_image.to(device_3)
                clean_v=clean_v.to(device_3)
...
def main(d_data_dir, i_data_dir,v_data_dir, project_name
        ...
        device_1="cuda:0", device_2="cuda:1", device_3="cuda:2", resume_from_checkpoint=True,i_block_num=2,v_block_num=2,i_expert_num=4,v_expert_num=4,i_topk_expert=2,v_topk_expert=2,i_alpha=1.0,v_alpha=1.0,f_block_num=3,mode="L", memory_efficient=True, load_optimizer_state=True, compress_checkpoints=True, save_optimizer_in_epoch_checkpoints=False):
    ...
    optimizer = torch.optim.Adam(trainable_parameters, lr=1e-4, betas=(0.9, 0.999), eps=1e-8)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device_3', type=str, default="cuda:2", help='第三个GPU设备')
