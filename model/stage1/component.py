...
class Noise_Multi_Expert_Decoder(nn.Module):
    def __init__(self, expert_num=4, topk_expert=2,mode="L"):
        super(Noise_Multi_Expert_Decoder, self).__init__()
...
if __name__=="__main__":
    '''# 创建测试数据
    Id_i = torch.rand(size=(2, 1, 224, 224)).to('cuda:0')
    Id_v = torch.rand(size=(2, 3, 224, 224)).to('cuda:1')
    ...
    # 测试多专家解码器
    print("\n=== 测试多专家解码器 ===")
    multi_expert_n_v = Noise_Multi_Expert_Decoder(expert_num=4, mode="RGB").to('cuda:1')
    multi_expert_n_i = Noise_Multi_Expert_Decoder(expert_num=4, mode="L").to('cuda:0')

    v_multi_decoded = multi_expert_n_v(v)
    i_multi_decoded = multi_expert_n_i(i)
    print(f"RGB多专家解码器输出形状: {v_multi_decoded.shape}")
    print(f"红外多专家解码器输出形状: {i_multi_decoded.shape}")
    
    # 显示专家数量
    print(f"\nRGB多专家解码器包含 {multi_expert_n_v.expert_num} 个专家")
    print(f"红外多专家解码器包含 {multi_expert_n_i.expert_num} 个专家")
    ...
    Id_i = Id_i.to('cuda:2')
    Id_v = Id_v.to('cuda:2')
    Ici=Id_i-i_multi_decoded
    Icv=Id_v-v_multi_decoded
    # 在通道维度（dim=1）拼接两个张量
