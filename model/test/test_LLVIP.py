...
def test_model(model,dataloader,device,task="dv_i"):
    ...
    with torch.no_grad():
        for i_batch, (i_data, v_data,d_data) in enumerate(tqdm(dataloader, desc="Testing")):
            ...
            if task=='dv_i':
                ...
                batch_metrics=Evaluator.evaluate_fusion_batch(out,clean,i_data)
            elif task=='di_v':
                out,clean=model(d_data,v_data)
                batch_metrics=Evaluator.evaluate_fusion_batch(out,v_data,clean)
...
def load_datasets(i_dir,v_dir,d_dir,transform_i,transform_v,transform_d,batch_size):
    ...
    # 退化图像根据任务类型选择模式 - 对于LLVIP，退化的可见光图像应该是RGB
...
def main():
    ...
    data_loader=load_datasets('/data/1024whs_data/DeMMI-RF/Test/Multi-Task/alltask/LLVIP/infrared',
                              '/data/1024whs_data/DeMMI-RF/Test/Multi-Task/alltask/LLVIP/visible',
                              '/data/1024whs_data/DeMMI-RF/Test/Multi-Task/alltask/LLVIP/input',
                              transform_i,transform_v,transform_d,batch_size=5)
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    model=DIV_fusion_model(task="dv_i").to(device)
    checkpoint_div = torch.load("/data/1024whs_checkpoint/CIV/Train_DIV_LLVIP_visible_noise15/latest_checkpoint.pth")
    model.load_state_dict(checkpoint_div['model_state_dict'])
    #checkpoint_iv = torch
    for test_time in range(5):
        with torch.no_grad():
            averaged_metrics=test_model(model,data_loader,device,task="dv_i")
        print("测试集上的平均指标:")
