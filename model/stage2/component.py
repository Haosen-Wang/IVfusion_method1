...
class Fusion_Net(nn.Module):
    def __init__(self,block_num=4):
        ...
        self.model=nn.Sequential()
        for i in range(block_num):
                self.model.add_module(f"restormer_block_{i+1}",Restormer(inp_channels=4,out_channels=1))
    def forward(self,x):
        x=self.model(x)
        return x
...
if __name__=="__main__":
    ...
    v_decoded = v_decoder(v)
    i_decoded = i_decoder(i)
    ...
    multi_expert_i = Global_Multi_Expert_Decoder(expert_num=4, mode="L").to('cuda:0')
