...
def test_model(model_noise,model_iv,dataloader,device):
    ...
    all_metrics = {}
    with torch.no_grad():
        for i_batch, (i_data, v_data) in enumerate(tqdm(dataloader, desc="Testing")):
            i_data = i_data.to(device)
            v_data = v_data.to(device)
            cv,_=model_noise(v_data)
            out,_=model_iv(cv,i_data)
            batch_metrics=Evaluator.evaluate_fusion_batch(out,cv,i_data)
...
def load_datasets(i_dir,v_dir,transform_i,transform_v,batch_size):
    ...
    paired_dataset = PairedDataset(i_dataset, v_dataset)
